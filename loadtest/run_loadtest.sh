...
#/ Examples: ./run_loadtest.sh -u wolfgang.moser@woodmark.de -pw Pa$$w0rd -p 10 -j https://hameet.cluster.local -d 180 -k ~/.ssh/id_rsa
#/ Options:
...
#/   -j, --jitsi-url: URL pointing to the Jitsi installation (optional, defaults to https://hameet.cluster.local)
...
if [ -z "$jitsiUrl" ]; then
  jitsiUrl="https://hameet.cluster.local"
fi
